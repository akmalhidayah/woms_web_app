...
class DashboardAdditionalGridUiTest extends TestCase
{
    ...
    public function test_dashboard_displays_overhaul_prognosis_and_top_ten_cost_grids(): void
    {
        ...
        $response->assertOk()
            ...
            ->assertSee('monthly-realization-panel flex min-h-[330px] flex-col rounded-xl border border-blue-600 bg-slate-100 p-3', false)
            ->assertSee('top-ten-panel flex min-w-0 flex-col rounded-xl border border-blue-600 bg-slate-100 p-3 xl:col-span-8', false)
            ->assertSee('overhaul-panel flex min-w-0 flex-col rounded-xl border border-amber-600 bg-slate-100 p-3 xl:col-span-4', false)
