    ...
    <div class="space-y-4">
        ...
        <section class="overflow-hidden rounded-[1.25rem] border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-4 py-3 overflow-x-auto">
                <nav class="mb-3" aria-label="Status set garansi">
                    <div class="flex min-w-max items-center gap-2">
                        @foreach ($tabOptions as $tabKey => $tabLabel)
                            ...
                            @endphp
                            <a
                                href="{{ route('admin.garansi.index', $tabQuery) }}"
                                @if ($isActiveTab) aria-current="page" @endif
                                class="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg border px-3 text-[10px] font-semibold transition {{ $isActiveTab ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50' }}"
                            >
                                <span>{{ $tabLabel }}</span>
                ...
                <table class="min-w-full text-[11px] text-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($garansiList as $g)
                            <tr data-garansi-order="{{ $g['order_id'] }}" class="transition hover:bg-slate-50">
                                <td class="px-4 py-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                    {{ $activeTab === 'set' ? 'Belum ada garansi yang sudah diset.' : 'Tidak ada garansi yang perlu tindakan.' }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="space-y-3 p-4 md:hidden">
                @forelse ($garansiList as $g)
                    <div data-garansi-order="{{ $g['order_id'] }}" class="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
