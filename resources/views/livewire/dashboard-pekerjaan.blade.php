    ...
    <div class="h-screen overflow-hidden">
        <div class="mx-auto flex h-full max-w-[1920px] flex-col px-4 py-4">
            ...
            <div class="grid min-h-0 flex-1 gap-4 xl:grid-cols-[1.45fr_1fr]">
                <section class="flex min-h-0 flex-col rounded-[1.6rem] border border-blue-200 bg-[linear-gradient(135deg,_#eff6ff_0%,_#ffffff_60%,_#dbeafe_100%)] p-5 shadow-sm">
                    <div class="mb-4 flex items-center justify-between gap-3">
                        <div class="text-[1.3rem] font-black text-blue-900">Regu Fabrikasi</div>
                        <span class="rounded-full bg-blue-100 px-3 py-1.5 text-[12px] font-semibold text-blue-700">{{ $fabrikasiTasks->count() }} item</span>
                    </div>

                    <div class="grid min-h-0 flex-1 gap-4 md:grid-cols-2">
                        @forelse ($fabrikasiPage as $task)
                            <article class="flex min-h-[220px] flex-col rounded-[1.35rem] border border-blue-100 bg-white p-4 shadow-sm">
                                <div class="text-[1.05rem] font-black leading-snug text-slate-900">{{ $task['job_name'] }}</div>
                                <div class="mt-1 text-[13px] font-semibold text-slate-500">{{ $task['notification_number'] ?: '-' }}</div>
                                <div class="mt-3 space-y-1 text-[13px] text-slate-600">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @foreach (($task['person_in_charge_profiles'] ?? []) as $profile)
                                            @php
                                                ...
                                                $avatar = is_array($profile) ? $avatarUrl($profile['avatar_path'] ?? null) : null;
                                            @endphp
                                            <span class="inline-flex items-center gap-2 rounded-full bg-slate-50 px-2.5 py-1.5 ring-1 ring-slate-200">
                                                @if ($avatar)
                                                    <img src="{{ $avatar }}" alt="" class="h-7 w-7 rounded-full object-cover">
                                                @else
                                                    <span class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-200 text-[11px] font-bold text-slate-700">{{ $initials($name) }}</span>
                                                @endif
                ...
                <section class="flex min-h-0 flex-col rounded-[1.6rem] border border-amber-200 bg-[linear-gradient(135deg,_#fff7ed_0%,_#ffffff_60%,_#fde68a_100%)] p-5 shadow-sm">
                    <div class="mb-4 flex items-center justify-between gap-3">
                        <div class="text-[1.3rem] font-black text-amber-900">Regu Bengkel (Refurbish)</div>
                        <span class="rounded-full bg-amber-100 px-3 py-1.5 text-[12px] font-semibold text-amber-700">{{ $refurbishTasks->count() }} item</span>
                    </div>

                    <div class="grid min-h-0 flex-1 gap-4">
                        @forelse ($refurbishPage as $task)
                            <article class="flex min-h-[220px] flex-col rounded-[1.35rem] border border-amber-100 bg-white p-4 shadow-sm">
                                <div class="text-[1.05rem] font-black leading-snug text-slate-900">{{ $task['job_name'] }}</div>
                                <div class="mt-1 text-[13px] font-semibold text-slate-500">{{ $task['notification_number'] ?: '-' }}</div>
                                <div class="mt-3 space-y-1 text-[13px] text-slate-600">
                                    <div>{{ $task['unit_work'] ?: '-' }}</div>
...
@else
    <div wire:poll.8s="nextSlide" class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
        <div class="mb-4 flex items-center justify-between gap-3">
            <div>
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                @foreach (($task['person_in_charge_profiles'] ?? []) as $profile)
                                    ...
                                    <span class="inline-flex items-center gap-2 rounded-full bg-slate-50 px-2 py-1 ring-1 ring-slate-200">
                                        @if ($avatar)
                                            <img src="{{ $avatar }}" alt="" class="h-6 w-6 rounded-full object-cover">
                                        @else
