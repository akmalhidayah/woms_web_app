        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" aria-label="Filter Stock Consumable">
            ...
            <form method="GET" action="{{ route('admin.appsheet.stock-consumable.index') }}" class="grid gap-3 md:grid-cols-3">
                <div>
                    ...
                    <input id="stock-consumable-search" name="search" value="{{ $filters['search'] }}" type="search" placeholder="UID, nama, ukuran, jenis, lokasi..." class="block w-full rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5 text-xs text-slate-700 outline-none transition focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-100">
                </div>
                <div>
                    <label for="stock-consumable-type" class="mb-1.5 block text-[11px] font-semibold text-slate-600">Jenis Consumable</label>
                    <select id="stock-consumable-type" name="jenis" class="block w-full rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5 text-xs text-slate-700 outline-none focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-100">
                ...
                <div>
                    ...
                    <select id="stock-consumable-status" name="status" class="block w-full rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5 text-xs text-slate-700 outline-none focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-100">
                ...
                <div class="flex flex-wrap items-center gap-3 md:col-span-3">
                    ...
                    <a href="{{ route('admin.appsheet.stock-consumable.index') }}" class="inline-flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-blue-600">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3.5 sm:px-5">
                <div>
                    ...
                    <p class="mt-0.5 text-[11px] text-slate-500">{{ number_format($rows->total(), 0, ',', '.') }} hasil dari {{ number_format($totalRows, 0, ',', '.') }} item consumable</p>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($rows as $row)
                            ...
                            <tr class="group align-middle text-slate-700 transition-colors hover:bg-blue-50/40">
                                <td class="min-w-80 px-5 py-4 align-top">
                                    <div class="flex items-start gap-3.5">
                                        <span class="relative inline-flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 text-slate-400 ring-1 ring-slate-200">
                                            <i data-lucide="package" class="h-5 w-5" aria-hidden="true"></i>
                                            @if ($row['_image_url'])
                                                <img src="{{ $row['_image_url'] }}" alt="Foto {{ $row['DESC.'] }}" loading="lazy" class="absolute inset-0 h-full w-full object-cover transition duration-200 group-hover:scale-105" onerror="this.remove()">
                                            @endif
                                        </span>
                                        <div class="min-w-0">
                                            ...
                                            <div class="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px]">
                                                <span class="font-mono font-bold text-blue-600">{{ $row['UID'] ?: 'Tanpa UID' }}</span>
