@php
    $googleConnectionIconOnly = $googleConnectionIconOnly ?? false;
    $hideGoogleReconnect = $hideGoogleReconnect ?? false;
@endphp
...
<div class="ml-auto flex shrink-0 flex-wrap items-center gap-2">
    @if ($googleConnected)
        <span
            @class([
                'inline-flex items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-xs font-semibold text-emerald-700',
                'h-9 w-9' => $googleConnectionIconOnly,
                'gap-1.5 px-3 py-2' => ! $googleConnectionIconOnly,
            ])
            @if ($googleConnectionIconOnly) title="Google terhubung" aria-label="Google terhubung" @endif
        >
            <i data-lucide="check-circle-2" class="h-4 w-4" aria-hidden="true"></i>
            @unless ($googleConnectionIconOnly)
                Google Terhubung
            @endunless
        </span>
        @if (! $hideGoogleReconnect && auth()->user()?->isSuperAdmin())
